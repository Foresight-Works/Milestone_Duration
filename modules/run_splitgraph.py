...
def neighbors_graph(node, G):
	...
	neighbors = list(G.neighbors(node))
	neighbors_as_edges = [(node, neighbour) for neighbour in neighbors]
...
G = nx.DiGraph(G)
partitions = split_graph(G, sparsity_threshold)
c = 0